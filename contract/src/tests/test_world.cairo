...
use dojo::model::{ModelStorage, ModelValueStorage, ModelStorageTest};
    ...
    fn test_setup_game() {
        ...
        let arg: u8 = 1;
        let board_id: u32 = actions_system.setup_game(arg);
        
        // let moves: Moves = world.read_model(caller);
        // let position: Position = world.read_model(caller);

        assert(board_id == 1, 'wrong board id');
        // assert(moves.remaining == 10, 'wrong initial moves');
        // assert(position.vec.x == 10 && position.vec.y == 10, 'wrong initial position');
    }
